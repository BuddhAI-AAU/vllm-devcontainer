...
class JsonConversationStore:
    ...
    def save_memory(self, conversation_id: str, summary: str):
        data = self._load()  # however you load your JSON
        if conversation_id not in data:
            data[conversation_id] = {}
        data[conversation_id]["memory_summary"] = summary
        self._save(data)

    def load_memory(self, conversation_id: str) -> str:
        data = self._load()
        return data.get(conversation_id, {}).get("memory_summary", "")
